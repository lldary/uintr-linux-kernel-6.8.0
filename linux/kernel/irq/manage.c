...
int irq_do_set_affinity_uintr(struct irq_data *data, const struct cpumask *mask,
			bool force)
{
	struct irq_desc *desc = irq_data_to_desc(data);
	struct irq_chip *chip = irq_data_get_irq_chip(data);
	...
	pr_info("Requested IRQ affinity change for IRQ %u: chip name: %s\n",
		data->irq, chip->name);
	/*
	 * Make sure we only provide online CPUs to the irqchip,
	 * unless we are being asked to force the affinity (in which
	 ...
	 */
	cpumask_and(&tmp_mask, prog_mask, cpu_online_mask);
	if(chip->irq_set_affinity_uintr == NULL) {
		pr_err("irq_set_affinity_uintr is not implemented for chip %s\n",
		       chip->name);
		raw_spin_unlock(&tmp_mask_lock);
		return -ENOSYS;
	}
	if (!force && !cpumask_empty(&tmp_mask))
		ret = chip->irq_set_affinity_uintr(data, &tmp_mask, force);
	else if (force)
		ret = chip->irq_set_affinity_uintr(data, mask, force);
	else
		ret = -EINVAL;
 ...
 */
 static int
 __setup_irq_uintr(unsigned int irq, struct irq_desc *desc, struct irqaction *new)
 {
	 struct irqaction *old, **old_ptr;
	 unsigned long flags, thread_mask = 0;
	 int ret, nested, shared = 0;
	 ...
	 if (!shared) {
		  ...
		  * will simply ignore that activation request.
		  */
		 pr_info("irq_activate_uintr\n");
		 ret = irq_activate_uintr(desc);
		 pr_info("irq_activate_uintr ret=%d\n", ret);
		 if (ret)
			 goto out_unlock;
...
/**
 *	request_threaded_irq_uintr - allocate an interrupt line
 *	@irq: Interrupt line to allocate
 *	@handler: Function to be called when the IRQ occurs.
 *		  Primary handler for threaded interrupts.
 *		  If handler is NULL and thread_fn != NULL
 ...
 *	IRQF_ONESHOT		Run thread_fn with interrupt line masked
 */
 int request_threaded_irq_uintr(unsigned int irq, irq_handler_t handler,
	irq_handler_t thread_fn, unsigned long irqflags,
	const char *devname, void *dev_id)
{
struct irqaction *action;
...
retval = __setup_irq_uintr(irq, desc, action);
...
void remove_to_open_uintr(struct irq_data* irq_data){
	...
	irq_do_set_affinity_uintr(irq_data, &cpu_mask, false);
	if(ret == -EBUSY){
