...
void irq_move_masked_irq_uintr(struct irq_data *idata, int cpu)
{
	...
	cpumask_clear(desc->pending_mask);
	cpumask_set_cpu(cpu, desc->pending_mask);
	irqd_set_move_pending(data);
	...
	pr_info("irq_move_masked_irq_uintr: irq %d to cpu %d\n", idata->irq, cpu);
	...
	if (!chip->irq_set_affinity_uintr)
		return;
...
void __irq_move_irq_uintr(struct irq_data *idata, int cpu)
{
	...
	pr_info("Moving UINTR %d to CPU %d\n", idata->irq, cpu);
	/*
	 * Be careful vs. already masked interrupts. If this is a
	 * threaded interrupt with ONESHOT set, we can end up with an
	 * interrupt storm.
	...
	if (!masked)
		idata->chip->irq_mask(idata);
	irq_move_masked_irq_uintr(idata, cpu);
	if (!masked)
		idata->chip->irq_unmask(idata);
}
